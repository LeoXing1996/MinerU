...
class SnipSameSideCornerRectangleShape(DefinitedBasicShape):
    NAME = 'SNIP_2_SAME_RECTANGLE'
    NUM_EDGES = 8
    NUM_CURVES = 0
    TRANSFORMS = [
        # [theta, flip_h, flip_v],
    ...
    def generate_segments(self, params):
        ...
        V2 = np.array([w / 2 - s2, -h / 2])
        V3 = np.array([w / 2, -h / 2 + s2])
        V4 = np.array([w / 2, h / 2 - s1])
    ...
    def draw_helper(self, prs, slide, params):
        ...
        shape = slide.shapes.add_shape(
            MSO_SHAPE.SNIP_2_SAME_RECTANGLE,  # snip same side corner rectangle
            left,
            top,
            width,
            height,
